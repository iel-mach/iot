...
if [ "$(uname)" = "Linux" ]; then
    ...
    if [ $? -eq 1 ]; then
        ...
        argocd app create guestbook --repo https://github.com/iel-mach/Inception-of-things.git --path 'p3/confs' --dest-server https://kubernetes.default.svc --dest-namespace 'dev' --grpc-web
        if [ $? -eq 20 ]; then
